...
PYBIND11_MODULE(paged_kvcache_ops, m) {
  ...
  py::class_<kvcache::GPUKVCacheMangerImpl>(m, "GPUKVCacheMangerImpl")
    ...
    .def("evict", &kvcache::GPUKVCacheMangerImpl::evict, py::arg("uid"), "Release all pages for a single user back to the empty pool")
    .def("get_empty_page_count", &kvcache::GPUKVCacheMangerImpl::get_empty_page_count, "Number of currently free (unallocated) cache pages")
    .def("get_user_page_count", &kvcache::GPUKVCacheMangerImpl::get_user_page_count)
    .def("get_active_page_limit", &kvcache::GPUKVCacheMangerImpl::get_active_page_limit)
